...
import static io.wisoft.capstonedesign.setting.data.DefaultFindOrderData.createDefaultOrder;
...
class FindOrderControllerTest {
    ...
    class CreateOrder {
        ...
        void create_order() throws Exception {
            ...
            CreateOrderRequest orderRequest = createDefaultOrder(infoId, findId, userId);
            String json = objectMapper.writeValueAsString(orderRequest);

            // expected
            mockMvc.perform(post("/api/find-orders/new")
    ...
    class FindOrder {
        ...
        void find_single_order() throws Exception {
            ...
            // expected
            mockMvc.perform(get("/api/find-orders/{id}", orderId)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken))
                    .andExpect(status().isOk())
        ...
        void find_single_order_fail1() throws Exception {
            ...
            // expected
            mockMvc.perform(get("/api/find-orders/{id}", 1000L)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken))
        ...
        void find_orders() throws Exception {
            ...
            CreateOrderRequest orderRequest1 = createDefaultOrder(infoId, findId, userId);
            findOrderService.save(orderRequest1);

            CreateOrderRequest orderRequest2 = createDefaultOrder(infoId, findId, userId);
            findOrderService.save(orderRequest2);

            // expected
            mockMvc.perform(get("/api/find-orders?page=0&size=5")
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken))
                    .andExpect(status().isOk())
    ...
    class UpdateOrder {
        ...
        void update_order() throws Exception {
            ...
            // expected
            mockMvc.perform(patch("/api/find-orders/{id}", orderId)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken)
                            .content(json))
                    .andExpect(status().isOk())
        ...
        void update_order_fail1() throws Exception {
            ...
            // expected
            mockMvc.perform(patch("/api/find-orders/{id}", 1000L)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken)
                            .content(json))
                    .andExpect(status().isNotFound())
    ...
    class DeleteOrder {
        ...
        void delete_order() throws Exception {
            ...
            // expected
            mockMvc.perform(delete("/api/find-orders/{id}", orderId)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken))
                    .andExpect(status().isOk())
                    .andDo(print());
        ...
        void delete_order_fail2() throws Exception {
            ...
            // expected
            mockMvc.perform(delete("/api/find-orders/{id}", 1000L)
                            .contentType(APPLICATION_JSON)
                            .header("Authorization", "bearer " + accessToken))
                    .andExpect(status().isNotFound())
                    .andDo(print());
